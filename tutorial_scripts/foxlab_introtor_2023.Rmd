...
I personally always like to preset my visualization enviroment by sourcing in 
...
# Analyzing and visulaizing your data
...
Now let's check the correlations amongst our variables. I am going to use the
...
If you really wanted to focus on on particular relation, such as the relation 
...
Maybe your curious if this relation varies by number of candy attributes.
```{r}
ggplot(candy_df, aes(sugarpercent, winpercent)) +
  geom_point() +
  geom_smooth(method = "lm", color = "black") +
...
```{r}
lm_nointeraction <- with(candy_df, lm(winpercent ~ sugarpercent + candyattributesum))
...
We can make a bar graph to visulalize this difference. I am going to use the
...
+
  labs(x = "", y = "Proportion Negative Affect", fill = "Interaction") +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank())
```

Maybe we decide this is the story we really want to tell: the superiority of 
chocolate vs fruity candies. We can dress up this figure to make it publication
ready by adding more infomative lables.
```{r}
ggplot(candy_chocolatevsfruitysummary_df, aes(type, winpercent, fill = type)) +
  geom_bar(stat = "summary", fun = "mean") +
  geom_errorbar(position = position_dodge(.9), width = .1, 
